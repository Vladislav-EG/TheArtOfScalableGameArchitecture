...
**SceneLoaderService** - сервис который отвечает за загрузку сцен в игре. Данный сервис позволяет создавать
уровни из сцен и загружать, выгружать нужные сцены в нужный момент. Аддитивные сцены являются невероятно
мощным и полезным инструментов при разработке игр, они обладают рядом следующих преимуществ:
- Полный контроль над тем что загружается и что выгружается;
- Уменьшают общее потребление памяти и позволяют удобно встроить Addressable;
- Обеспечивают детализацию для командной работы и позволяют избежать конфликтов слияния;

## SceneLoaderService

Отличный пример SceneLoaderService - [Advanced Scene Manager](https://assetstore.unity.com/packages/tools/utilities/advanced-scene-manager-174152?srsltid=AfmBOorbVN07VZpI_iMK8kcedA4OC3MseMa-Jm073xJnyZtUJhvk2hRj).
Это ассет в UnityAssetStore, который предоставляет удобный, многофункциональный, гибкий менеджер сцен.

Как я вижу хороший менеджер сцен - это монобехевиор сервис, который находится в BootstrapScene, в него можно
...
запускать нужный уровнь в данный момент в редакторе, это позволит обеспечить комфортный дебаг уровня. 
Галочка выбора активной сцены. При загрузке уровня в котором некоторые сцены будут повторяться, эти сцены не выгружаются
а остаются.

## TIPS

Еще есть одна интересная особенность, что мы не можем сделать сцену активной в этом же кадре, нужно подождать
один кадр пока она загрузится, решение этой проблемы и пример загрузчика можно посмотреть тут:
...
> [!TIP]
> При реализации SceneLoaderService стоит сразу реализовать систему которая позволит запускать нужные в 
> данный момент сцены в редакторе.
